...
**Execution:** audit each batch following `~/.claude/skills/api-audit/agent-prompt.md` instructions and output format. Evaluate sequentially, one batch at a time.
```
Task(
  // Parallel optimization (Claude Code only): one agent per batch, max 6 concurrent
  subagent_type: "general-purpose",
  model: "sonnet",  // Opus for DEEP tier
