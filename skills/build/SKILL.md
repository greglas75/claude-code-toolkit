...
Run Q1-Q17 self-eval (from `~/.claude/rules/testing.md`) on each test file you wrote or modified.
...
✅/❌  TYPES: type checker passes — `tsc --noEmit` (TS), `mypy`/`pyright` (Python), `go vet` (Go), or skipped if none configured
