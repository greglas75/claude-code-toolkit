...
For large reviews, `/review` splits audit steps across 2 custom agents in parallel. Agents are defined in `~/.claude/skills/review/agents/` with enforced read-only tool access (no Write/Edit) and optimized model routing.
