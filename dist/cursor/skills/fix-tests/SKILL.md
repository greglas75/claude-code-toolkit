...
```
1. [x]/[ ]  ~/.cursor/test-patterns.md         -- Q1-Q17 protocol, lookup table, scoring
2. [x]/[ ]  ~/.cursor/test-patterns-catalog.md  -- G-*/P-* pattern definitions (grep matched IDs)
3. [x]/[ ]  ~/.cursor/rules/testing.md         -- quality gates, Batch Diagnosis greps
```

**Conditional (load only when pattern matches):**
```
4. [x]/[ ]/SKIP  ~/.cursor/test-patterns-redux.md    -- LOAD when pattern is P-40, P-41, P-44, G-41-G-45
5. [x]/[ ]/SKIP  ~/.cursor/test-patterns-nestjs.md   -- LOAD when pattern is NestJS-AP1, NestJS-P1-P3, G-33, G-34
```
...
Run from `~/.cursor/rules/testing.md` to quantify scope. Report counts BEFORE fixing.
...
RESOLVED PATHS (use these, not hardcoded):
  Rules: [RESOLVED_RULES_PATH]       -- resolved from ~/.cursor/rules/ or _agent/rules/
  Patterns: [RESOLVED_PATTERNS_PATH] -- resolved from ~/.cursor/ or _agent/
