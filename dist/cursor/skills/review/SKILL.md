...
2. [x]/[ ]  ~/.cursor/rules/code-quality.md         -- CQ1-CQ20 (skip if TIER 0)
```
...
|------|-----------|-----------|
| `~/.cursor/review-protocol.md` | TIER 2+ | TIER 0 or TIER 1 |
| `~/.cursor/rules/testing.md` | TIER 2+, OR diff contains `*.test.*`/`*.spec.*` files | TIER 0-1 with no test files |
| `~/.cursor/test-patterns.md` | Diff contains `*.test.*`/`*.spec.*` files | No test files in diff |
| `~/.cursor/rules/security.md` | Triage Step 6 flags SECURITY conditional section, OR TIER 3 | TIER 0-2 with no security signals |
