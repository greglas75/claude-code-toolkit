...
```
1. check/X  ~/.antigravity/rules/code-quality.md         -- CQ1-CQ20 production code checklist
2. check/X  ~/.antigravity/rules/testing.md              -- Q1-Q17 test self-eval checklist
3. check/X  ~/.antigravity/test-patterns.md              -- Q1-Q17 protocol, lookup table -> routes to catalog/domain files
4. check/X  ~/.antigravity/rules/file-limits.md          -- 250-line file limit, 50-line function limit
```
...
1. Read project `CLAUDE.md` and `.antigravity/rules/` (or `_agent/rules/`) for conventions
...
**Agent 1: Blast Radius Mapper** -- uses `~/.antigravity/skills/build/agents/dependency-mapper.md`

Delegate to @dependency-mapper to trace blast radius:
- FEATURE: [description]
- TARGET FILES: [files the feature will likely touch]
- PROJECT ROOT: [cwd]
- INSTRUCTIONS: Read `~/.antigravity/skills/build/agents/dependency-mapper.md` for full protocol. Trace all importers/callers of the target files. Identify what might break or need updates. Read project CLAUDE.md for import conventions.

**Agent 2: Existing Code Scanner** -- uses `~/.antigravity/skills/build/agents/existing-code-scanner.md`

Delegate to @existing-code-scanner to find overlapping code:
- FEATURE: [description]
- PLANNED NEW CODE: [functions/components/services to create]
- PROJECT ROOT: [cwd]
- INSTRUCTIONS: Read `~/.antigravity/skills/build/agents/existing-code-scanner.md` for full protocol. Search for existing services/helpers/components similar to what's planned. Prevent duplication. Read project CLAUDE.md for file organization conventions.
...
[Read limits from ~/.antigravity/rules/file-limits.md or project CLAUDE.md override]
...
- **Follow project conventions** -- from CLAUDE.md and `.antigravity/rules/`
...
Run CQ1-CQ20 self-eval (from `~/.antigravity/rules/code-quality.md`) on each production file you wrote/modified.
...
- Follow patterns from `~/.antigravity/test-patterns.md`
- Run tests: verify GREEN

### 3.5: Test Self-Eval (before Phase 4)

Run Q1-Q17 self-eval (from `~/.antigravity/rules/testing.md`) on each test file you wrote or modified.
...
- INSTRUCTIONS: Read `~/.antigravity/skills/build/agents/test-quality-auditor.md` for full protocol. Verify 11 hard gates and run 17-question self-eval (Q1-Q17) on each test file. Read project CLAUDE.md and `.antigravity/rules/` for project-specific test conventions.
