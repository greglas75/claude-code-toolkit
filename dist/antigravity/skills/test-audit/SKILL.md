...
```
1. [x]/[ ]  ~/.antigravity/test-patterns.md              -- Q1-Q17 protocol, lookup table -> routes to catalog/domain files
2. [x]/[ ]  ~/.antigravity/rules/testing.md              -- iron rules, test requirements by code type, self-eval
```
...
If running in Antigravity, Cursor, or other IDEs where `~/.antigravity/` is not accessible, resolve paths from `_agent/` in project root:
- `~/.antigravity/test-patterns.md` -> `_agent/test-patterns.md`
- `~/.antigravity/rules/testing.md` -> `_agent/rules/testing.md`
...
After grouping, split files into batches of 6-8. For each batch, evaluate against Q1-Q17 using the instructions below.
...
1. **Fix** -- user says "napraw X" or "fix tier D files" -> rewrite test files following `~/.antigravity/test-patterns.md`
