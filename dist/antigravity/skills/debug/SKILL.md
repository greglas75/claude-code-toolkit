...
Run Q1-Q17 self-eval (from `~/.antigravity/rules/testing.md`) on the regression test. Critical gates must pass.

### 4.6: CQ self-eval (if production code changed)

Run CQ1-CQ20 (from `~/.antigravity/rules/code-quality.md`) on each modified production file. Critical gates must pass.
