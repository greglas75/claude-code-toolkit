...
```
1. check/X  ~/.antigravity/skills/refactor/rules.md      -- types, iron rules, hard gates, scope fence, sub-agents
2. check/X  ~/.antigravity/refactoring-protocol.md       -- full ETAP-1A -> 1B -> 2 protocol
3. check/X  ~/.antigravity/rules/code-quality.md         -- CQ1-CQ20 production code checklist
4. check/X  ~/.antigravity/rules/testing.md              -- Q1-Q17 test self-eval checklist
5. check/X  ~/.antigravity/test-patterns.md              -- Q1-Q17 protocol, lookup table -> routes to catalog/domain files
```
...
Resolve paths from both possible locations -- try `~/.antigravity/` first, fall back to `_agent/` in project root:
- `~/.antigravity/skills/` or `_agent/skills/`
- `~/.antigravity/rules/` or `_agent/rules/`
- `~/.antigravity/refactoring-protocol.md` or `_agent/refactoring-protocol.md`
- `~/.antigravity/refactoring-god-class.md` or `_agent/refactoring-god-class.md`
- `~/.antigravity/refactoring-examples/` or `_agent/refactoring-examples/`
- `~/.antigravity/test-patterns.md` or `_agent/test-patterns.md`
...
Follow `~/.antigravity/rules/stack-detection.md` to detect language and test runner.

Then load the appropriate example module:

| Stack + Runner | Read |
|----------------|------|
| Python + pytest | `~/.antigravity/refactoring-examples/python-pytest.md` |
| Python + unittest | `~/.antigravity/refactoring-examples/python-unittest.md` |
| TypeScript + Vitest | `~/.antigravity/refactoring-examples/typescript-vitest.md` |
| TypeScript + Jest | `~/.antigravity/refactoring-examples/typescript-jest.md` |
| React + RTL | `~/.antigravity/refactoring-examples/react-rtl.md` |
| NestJS | `~/.antigravity/refactoring-examples/nestjs-testing.md` |
...
**Agent 1: Dependency Mapper** -- uses `~/.antigravity/skills/refactor/agents/dependency-mapper.md`

Delegate to @dependency-mapper to trace blast radius:
- TARGET FILES: [list from Phase 1]
- PROJECT ROOT: [cwd]
- INSTRUCTIONS: Read `~/.antigravity/skills/refactor/agents/dependency-mapper.md` for full protocol. Trace all importers/callers of each target file. Build a dependency map showing blast radius. Read project CLAUDE.md for import conventions.

**Agent 2: Existing Code Scanner** -- uses `~/.antigravity/skills/refactor/agents/existing-code-scanner.md`

Delegate to @existing-code-scanner to find overlapping code:
- PLANNED EXTRACTIONS: [list of functions/methods to extract]
- PROJECT ROOT: [cwd]
- INSTRUCTIONS: Read `~/.antigravity/skills/refactor/agents/existing-code-scanner.md` for full protocol. Search for existing services/helpers similar to planned extraction targets. Read project CLAUDE.md for file organization conventions.
...
```
Read ~/.antigravity/refactoring-protocol.md

# ONLY if detected type = GOD_CLASS:
Read ~/.antigravity/refactoring-god-class.md
```
...
**Agent 3: Test Quality Auditor** -- uses `~/.antigravity/skills/refactor/agents/test-quality-auditor.md`
...
- INSTRUCTIONS: Read `~/.antigravity/skills/refactor/agents/test-quality-auditor.md` for full protocol. Verify all 11 hard gates and run the 17-question self-eval on each test file. Read project CLAUDE.md and `.antigravity/rules/` for project-specific test conventions.
...
**Agent 4: Post-Extraction Verifier** -- uses `~/.antigravity/skills/refactor/agents/post-extraction-verifier.md`
...
- REFACTORING TYPE: [type]
- INSTRUCTIONS: Read `~/.antigravity/skills/refactor/agents/post-extraction-verifier.md` for full protocol. Verify delegation applied, imports updated, file sizes reduced, no orphaned code. Read project CLAUDE.md and `.antigravity/rules/` for project-specific limits.
...
2. **If file doesn't exist**: create it with the template from `~/.antigravity/skills/review/rules.md`
