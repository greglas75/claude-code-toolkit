...
Full protocol (ETAP-1A -> 1B -> 2) is at `~/.antigravity/refactoring-protocol.md` -- read on-demand when `/refactor` starts.
Stack-specific examples are at `~/.antigravity/refactoring-examples/{stack}.md` -- loaded after auto-detection.
...
Before writing tests: read `~/.antigravity/test-patterns.md` (global). Classify code type, load matching patterns from lookup table, apply them.
...
Agent definition files: `~/.antigravity/skills/refactor/agents/*.md`
...
Read project CLAUDE.md for overrides. Defaults: `~/.antigravity/rules/file-limits.md`.
