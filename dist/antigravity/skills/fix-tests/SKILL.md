---
name: fix-tests
description: "Batch repair of systematic test quality issues. Runs Batch Diagnosis greps, identifies affected files, fixes per pattern (parallel agents in Claude Code, sequential inline in other environments). Use: /fix-tests --pattern P-41 [path] or /fix-tests --triage. NOT when tests need full rewrite (use /refactor or /test-audit first)."
user-invocable: true
---
...
```
1. [x]/[ ]  ~/.antigravity/test-patterns.md         -- Q1-Q17 protocol, lookup table, scoring
2. [x]/[ ]  ~/.antigravity/test-patterns-catalog.md  -- G-*/P-* pattern definitions (grep matched IDs)
3. [x]/[ ]  ~/.antigravity/rules/testing.md         -- quality gates, Batch Diagnosis greps
```

**Conditional (load only when pattern matches):**
```
4. [x]/[ ]/SKIP  ~/.antigravity/test-patterns-redux.md    -- LOAD when pattern is P-40, P-41, P-44, G-41-G-45
5. [x]/[ ]/SKIP  ~/.antigravity/test-patterns-nestjs.md   -- LOAD when pattern is NestJS-AP1, NestJS-P1-P3, G-33, G-34
```
...
This skill uses inline fixer execution (Step 4). **If inline analysis is not available** (Cursor, Codex, Antigravity):
- Skip all "Spawn via inline analysis" blocks -- fix files inline, sequentially
...
Run from `~/.antigravity/rules/testing.md` to quantify scope. Report counts BEFORE fixing.
...
Split file pairs into batches of 5. For each batch, process each batch inline.
...
RESOLVED PATHS (use these, not hardcoded):
  Rules: [RESOLVED_RULES_PATH]       -- resolved from ~/.antigravity/rules/ or _agent/rules/
  Patterns: [RESOLVED_PATTERNS_PATH] -- resolved from ~/.antigravity/ or _agent/
...
|------|---------|--------|-------|-------|--------|
| profileSlice.test.ts | P-41 | 6/17 | 14/17 | [x] | FIXED |
| clientProfileFilter.test.tsx | G-43 | 8/17 | 15/17 | [x] | FIXED |
| industrySlice.test.ts | P-41 | 5/17 | 13/17 | [x] | NEEDS_REVIEW |
```
