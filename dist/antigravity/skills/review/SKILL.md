...
```
1. [x]/[ ]  ~/.antigravity/skills/review/rules.md        -- Iron Rules, severity, confidence, backlog
2. [x]/[ ]  ~/.antigravity/rules/code-quality.md         -- CQ1-CQ20 (skip if TIER 0)
```
...
|------|-----------|-----------|
| `~/.antigravity/review-protocol.md` | TIER 2+ | TIER 0 or TIER 1 |
| `~/.antigravity/rules/testing.md` | TIER 2+, OR diff contains `*.test.*`/`*.spec.*` files | TIER 0-1 with no test files |
| `~/.antigravity/test-patterns.md` | Diff contains `*.test.*`/`*.spec.*` files | No test files in diff |
| `~/.antigravity/rules/security.md` | Triage Step 6 flags SECURITY conditional section, OR TIER 3 | TIER 0-2 with no security signals |
...
Read `memory/backlog.md` from the project's auto memory directory (path shown in system prompt). If the file doesn't exist, create it from the table template in rules.md (table format: `| ID | Fingerprint | File | Issue | ... |`). Check if any OPEN items are in files touched by this PR. If yes, include them in the report under `## BACKLOG ITEMS IN SCOPE`.
...
**Structure Auditor** -- `~/.antigravity/skills/review/agents/structure-auditor.md` (Sonnet, read-only):
```
Analysis to perform (execute inline):
  prompt: "Read your full instructions at ~/.antigravity/skills/review/agents/structure-auditor.md, then audit these changed files.
...
**Behavior Auditor** -- `~/.antigravity/skills/review/agents/behavior-auditor.md` (Sonnet or Opus, read-only):
```
Analysis to perform (execute inline):
  model: [use model from Behavior Auditor Model Routing above]
  prompt: "Read your full instructions at ~/.antigravity/skills/review/agents/behavior-auditor.md, then audit these changed files.
...
### Step Reference (details in ~/.antigravity/review-protocol.md)
...
- 2.4: SRP, coupling, file/function size limits (250 lines source / 400 tests, 50 line functions -- see `~/.antigravity/rules/file-limits.md`)
...
3. Spawn **Confidence Re-Scorer** -- `~/.antigravity/skills/review/agents/confidence-rescorer.md` (Haiku):

```
Analysis to perform (execute inline):
  prompt: "Read your full instructions at ~/.antigravity/skills/review/agents/confidence-rescorer.md, then re-score these issues.
...
2. Present each question to the user -- max 4 at a time (tool limit):
   - Header: "Author input needed"
   - Each question is presented with options relevant to the question
...
If MODE 1 (report only): after report + backlog update, prompt the user for next action:
...
After applying fixes and before committing, verify ALL of these. Print each with [x]/[ ]:
...
Changes are already committed and tagged (auto-commit in Phase B). Prompt the user:
