...
**IMPORTANT:** Read the project's `CLAUDE.md` and `.antigravity/rules/` directory at the start of your audit to learn project-specific limits (file sizes, naming conventions, architecture patterns).
...
- Files exceeding project's line limits (check `.antigravity/rules/` -- flag NEW violations only)
...
8. **READ PROJECT RULES** -- always read `CLAUDE.md` and `.antigravity/rules/` at the start. Project-specific limits override defaults.
