...
When coding directly (1-2 files, no `/build` or `/refactor`), still run CQ1-CQ20 self-eval from `~/.antigravity/rules/code-quality.md` on each production file before writing tests. Critical gate (static + conditional), evidence requirement, and thresholds apply.
