...
Full protocol (ETAP-1A -> 1B -> 2) is at `~/.codex/refactoring-protocol.md` -- read on-demand when `/refactor` starts.
Stack-specific examples are at `~/.codex/refactoring-examples/{stack}.md` -- loaded after auto-detection.
...
Before writing tests: read `~/.codex/test-patterns.md` (global). Classify code type, load matching patterns from lookup table, apply them.
...
Analysis prompts are in `references/*.md`. Read the relevant reference file and perform the analysis inline.
## Modes
...
Read project CLAUDE.md for overrides. Defaults: `~/.codex/rules/file-limits.md`.
