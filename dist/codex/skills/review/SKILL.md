...
**Structure Auditor** -- `references/structure-auditor.md` (Sonnet, read-only):
Read `references/structure-auditor.md` and perform this analysis yourself.


**Behavior Auditor** -- `references/behavior-auditor.md` (Sonnet or Opus, read-only):
Read `references/behavior-auditor.md` and perform this analysis yourself.
...
### Step Reference (details in ~/.codex/review-protocol.md)
...
- 2.4: SRP, coupling, file/function size limits (250 lines source / 400 tests, 50 line functions -- see `~/.codex/rules/file-limits.md`)
...
3. Spawn **Confidence Re-Scorer** -- `references/confidence-rescorer.md` (Haiku):

Read `references/confidence-rescorer.md` and perform this analysis yourself.
...
2. Use ask the user to surface each question -- max 4 at a time (tool limit):
   - Header: "Author input needed"
   - Each question becomes one ask the user question with options relevant to the question
...
If MODE 1 (report only): after report + backlog update, use ask the user to prompt for next action:
...
Changes are already committed and tagged (auto-commit in Phase B). Use ask the user to prompt:
