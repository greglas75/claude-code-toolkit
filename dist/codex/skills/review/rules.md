...
Full protocol with detailed checklists, red-flag patterns, and report templates is at `~/.codex/review-protocol.md` -- read it on-demand when `/review` starts.
...
For larger reviews, `/review` splits audit steps across 2 custom agents in parallel. Agents are defined in `references/` with enforced read-only tool access (no Write/Edit) and optimized model routing.
...
- **Before writing tests:** read `~/.codex/test-patterns.md` (global, all projects). Apply all matching patterns (check WHEN triggers against code under test).
- **After writing tests:** run Q1-Q17 self-eval checklist (17 yes/no questions, per `~/.codex/rules/testing.md`). Score < 14 = fix before continuing. Critical gate: Q7, Q11, Q13, Q15, Q17.
- **When user gives feedback about test gaps:** append new pattern to `~/.codex/test-patterns.md` with WHEN trigger, required tests, and source.
...
- Detailed checklists for each stack in `~/.codex/review-protocol.md`
...
To find it: look for the `auto memory directory` path in your system prompt (e.g., `~/.codex/projects/{project-slug}/memory/`). The backlog file is `backlog.md` inside that directory.
