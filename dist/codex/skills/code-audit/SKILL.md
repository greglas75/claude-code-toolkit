...
```
1. [x]/[ ]  ~/.codex/rules/code-quality.md         -- CQ1-CQ20 checklist, scoring, evidence, N/A rules, examples
2. [x]/[ ]  ~/.codex/rules/security.md             -- SSRF, XSS, auth, path traversal patterns
3. [x]/[ ]  ~/.codex/rules/file-limits.md          -- 250-line file limit, 50-line function limit
```
...
**Pass this context to every batch agent** as `PROJECT_CONTEXT` at the top of the file list.

## Step 3: Parallel Evaluation

Split files into batches of 6-8. For each batch, evaluate each batch with this prompt:
...
N/A HANDLING: See ~/.codex/rules/code-quality.md for strict N/A rules.
...
2. **If file doesn't exist**: create it with this template (or use the one in `~/.codex/skills/review/rules.md` if available):
   ```markdown
   # Tech Debt Backlog
...
After showing the report, **automatically propose** what to do next. Use ask the user with options based on audit results.
...
Use ask the user if available, otherwise print as plain text and wait for response.
...
- Use **Sonnet** for `--quick` mode (binary checks sufficient)
- Use **Opus** for `--deep` mode (evidence + nuanced CQ4/CQ14/CQ20 detection)
- Max 6 parallel agents for batch evaluation
- Each agent gets 6-8 files per batch (production files are larger than test files)
- Total time estimate: ~3 min for quick (50 files), ~15 min for deep (50 files)
- **Always read project CLAUDE.md first** -- project-specific limits override defaults
- If project has no CLAUDE.md, use defaults from `~/.codex/rules/file-limits.md`
